...
-- Add indexes
CREATE INDEX IF NOT EXISTS idx_owner_payouts_office ON owner_payouts(office_id);
CREATE INDEX IF NOT EXISTS idx_owner_payouts_owner ON owner_payouts(owner_id);
CREATE INDEX IF NOT EXISTS idx_owner_payouts_unit ON owner_payouts(unit_id);
CREATE INDEX IF NOT EXISTS idx_owner_payouts_status ON owner_payouts(status);
CREATE INDEX IF NOT EXISTS idx_owner_payouts_date ON owner_payouts(payout_date);

CREATE INDEX IF NOT EXISTS idx_office_finances_office ON office_finances(office_id);
CREATE INDEX IF NOT EXISTS idx_office_finances_type ON office_finances(type);
CREATE INDEX IF NOT EXISTS idx_office_finances_category ON office_finances(category);
CREATE INDEX IF NOT EXISTS idx_office_finances_date ON office_finances(date);
CREATE INDEX IF NOT EXISTS idx_office_finances_status ON office_finances(status);
...
CREATE POLICY "Users can delete owner payouts from their office" ON owner_payouts
FOR DELETE USING (
    office_id IN (
        SELECT office_id FROM users WHERE auth_id = auth.uid()::text
    )
...
CREATE POLICY "Users can delete office finances from their office" ON office_finances
FOR DELETE USING (
    office_id IN (
        SELECT office_id FROM users WHERE auth_id = auth.uid()::text
    )
